...
void CalculatePixelValue(int dimension, int samples,vector<sphere>& spheres,
		int start, int end,vector<vec>& pixels, ray& camera, vec& cx,vec& cy, _Binder<_Unforced, uniform_real_distribution<double>&, default_random_engine&>& get_random_number,vec r)
{
...
int main(int argc, char **argv)
{
	...
	for (int i = 0; i < 100; i++)
	{
		...
		for (int i = 0; i < 1; i++)
		{
			int length = dimension / threadNum;
			int start = i*length;
			int end = ((i + 1)*length) - 1;
			threadList.push_back(thread(CalculatePixelValue,dimension,samples,spheres,0,dimension,std::ref(pixels),camera,cx,cy,get_random_number,r));
		}
		// Collect threads.
