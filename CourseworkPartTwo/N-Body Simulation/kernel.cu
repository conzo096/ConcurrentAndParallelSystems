...
//
//auto data_size = sizeof(float) * MAXPARTICLES;
//// create buffers.
...
//
//float fx[MAXPARTICLES];
//float fy[MAXPARTICLES];
//float fz[MAXPARTICLES];
//
//float mass[MAXPARTICLES];
//
//// Declare buffers.
...
//__global__ void updateParticles(const float *mass, float *px, float *py, float *pz,
//						float *vx, float *vy, float *vz,	
//						const float *fx, const float *fy, const float *fz, const double *deltaTime)
//{
...
//
//	px[idx] += deltaTime[0] * vx[idx];
//	py[idx] += deltaTime[0] * vy[idx];
//	pz[idx] += deltaTime[0] * vz[idx];
//}
//
...
//
//		// Update Cuda Buffers.
//		fx[i] = ParticlesContainer[i].force.x;
//		fz[i] = ParticlesContainer[i].force.y;
//		fy[i] = ParticlesContainer[i].force.z;
//	}
//}
//
//
...
//	//}
//
//	// GPU already has updated positions and velocity. Send new force to it.
...
//
//	// copy deltaTime over.
//	cudaMemcpy(buffer_deltaTime, &deltaTime, sizeof(double), cudaMemcpyHostToDevice);
//
//	// Execute kernel.
//	updateParticles<<<8, MAXPARTICLES/8>>>(buffer_mass, buffer_px, buffer_py, buffer_pz,
//		buffer_vx, buffer_vy, buffer_vz,
//		buffer_fx, buffer_fy, buffer_fz,buffer_deltaTime);
...
//	for (int i = 0; i < MAXPARTICLES; i++)
//	{
//		ParticlesContainer[i].pos = glm::vec3(px[i], py[i], pz[i]);
//		ParticlesContainer[i].velocity = glm::vec3(vx[i], vy[i], vz[i]);
//
...
//		py[i] = y;
//		pz[i] = z;
//		mass[i] = ParticlesContainer[i].mass;
//	}
//
...
//	cudaMalloc((void**)&buffer_deltaTime, sizeof(double));
//
//	// Send data to GPU.
